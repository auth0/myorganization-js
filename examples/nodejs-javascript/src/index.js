...
async function getOrganizationDetails() {
  ...
  try {
    ...
    console.log(`   ID: ${details.organization?.id}`);
    console.log(`   Name: ${details.organization?.name}`);
    console.log(`   Display Name: ${details.organization?.display_name || '-'}`);
...
async function createDomain(domainName) {
  ...
  try {
    ...
    console.log(`   ID: ${result.organization_domain?.id}`);
    console.log(`   Domain: ${result.organization_domain?.domain}`);
    console.log(`   Status: ${result.organization_domain?.status}`);
    
    if (result.organization_domain?.status === 'pending') {
      console.log('\n📝 Verification Instructions:');
      console.log(`   Add TXT record to your DNS:`);
      console.log(`   Host: ${result.organization_domain?.verification_host}`);
      console.log(`   Value: ${result.organization_domain?.verification_txt}`);
    }
...
async function createOIDCProvider(config) {
  ...
  try {
    ...
    console.log(`   ID: ${result.identity_provider?.id}`);
    console.log(`   Name: ${result.identity_provider?.display_name}`);
