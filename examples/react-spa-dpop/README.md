# MyOrganization SDK - React SPA Example with DPoP

React application demonstrating Auth0 MyOrganization SDK integration with `@auth0/auth0-react` using **DPoP (Demonstrating Proof-of-Possession)** for enhanced token security.

DPoP binds access tokens to a cryptographic key pair, preventing token theft and replay attacks. The Auth0 React SDK's `createFetcher()` handles DPoP proof generation, nonce management, and automatic retries transparently.
...
> When DPoP is enabled, the API will reject Bearer tokens that are not accompanied by a valid DPoP proof. The Auth0 React SDK handles proof generation automatically via `createFetcher()`.
...
### Auth0Provider with DPoP

Enable DPoP on the `Auth0Provider`:

```typescript
{/* useDpop: generates an ES256 key pair and binds tokens to it */}
{/* cacheLocation: persists tokens in localStorage so sessions survive page refreshes */}
<Auth0Provider
...
### Custom hook with DPoP fetcher

The `useMyOrganization` hook creates a `MyOrganizationClient` using `createFetcher()` for DPoP proof generation, nonce management, and automatic retries.
...
export function useMyOrganization() {
    ...
    const client = useMemo(() => {
        ...
        // 2. Handles server nonce challenges (extracts dpop-nonce header, retries)
...
| `invalid_dpop_proof` error                  | The DPoP proof JWT is malformed or the key doesn't match.                                        | Ensure `useDpop={true}` is set on `Auth0Provider`. Clear browser storage and try again — the DPoP key pair may be corrupted.                                                                                                                  |
